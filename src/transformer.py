...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x):
        ...
        div = (x - mean) / torch.sqrt(var + self.epsilon)
        return self.alpha * div + self.bias
...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h: int, dropout: float):
        ...
        self.d_model = d_model
        self.h = h
        assert d_model % h == 0, "d_model no es divisible por h"
    ...
    @staticmethod
    def attention(query, key, value, mask: torch.Tensor, dropout: nn.Dropout):
        d_k = query.shape[-1]
...
def build_transformer(
    ...
    # create the decoder and the decoder
